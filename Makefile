...
# okofw-example.openstack.org/example-operator-bundle:$VERSION and okofw-example.openstack.org/example-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= okofw-example.openstack.org/example-operator
...
test: manifests generate fmt vet envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" ginkgo run -v --trace $(GINKGO_ARGS) ./...
